...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_events_tx.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_events_tx TYPE STANDARD TABLE OF zhms_tx_events,
      ls_events_tx LIKE LINE OF lt_events_tx.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        MOVE waxls-value TO ls_events_tx-natdc.
      WHEN '2'.
        MOVE waxls-value TO ls_events_tx-typed.
      WHEN '3'.
        MOVE waxls-value TO ls_events_tx-loctp.
      WHEN '4'.
        MOVE waxls-value TO ls_events_tx-event.
      WHEN '5'.
        MOVE waxls-value TO ls_events_tx-spras.
      WHEN '6'.
        MOVE waxls-value TO ls_events_tx-denom.
        APPEND ls_events_tx TO lt_events_tx.
        CLEAR ls_events_tx .
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tx_events FROM TABLE lt_events_tx.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
