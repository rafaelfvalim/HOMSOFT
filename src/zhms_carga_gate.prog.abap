...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_gate.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_tb_gate TYPE STANDARD TABLE OF zhms_tb_gate,
      ls_tb_gate LIKE LINE OF lt_tb_gate.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        IF ls_tb_gate IS NOT INITIAL.
          APPEND ls_tb_gate TO lt_tb_gate.
          CLEAR ls_tb_gate.
        ENDIF.
        MOVE waxls-value TO ls_tb_gate-gate.
      WHEN '2'.
        MOVE waxls-value TO ls_tb_gate-defau.
        APPEND ls_tb_gate TO lt_tb_gate.
        CLEAR ls_tb_gate.
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tb_gate FROM TABLE lt_tb_gate.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
