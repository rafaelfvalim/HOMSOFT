...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_gate_tx.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_tx_gate TYPE STANDARD TABLE OF zhms_tx_gate,
      ls_tx_gate LIKE LINE OF lt_tx_gate.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        IF ls_tx_gate IS NOT INITIAL.
          APPEND ls_tx_gate TO lt_tx_gate.
          CLEAR ls_tx_gate.
        ENDIF.
        MOVE waxls-value TO ls_tx_gate-gate.
      WHEN '2'.
        MOVE waxls-value TO ls_tx_gate-spras.
      WHEN '3'.
        MOVE waxls-value TO ls_tx_gate-denom.
        APPEND ls_tx_gate TO lt_tx_gate.
        CLEAR ls_tx_gate.
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tx_gate FROM TABLE lt_tx_gate.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
