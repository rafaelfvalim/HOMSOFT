...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_grpfld.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_grpfld TYPE STANDARD TABLE OF zhms_tb_grpfld,
      ls_grpfld LIKE LINE OF lt_grpfld.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        MOVE waxls-value TO ls_grpfld-codgf.
        APPEND ls_grpfld TO lt_grpfld.
        CLEAR ls_grpfld.
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tb_grpfld FROM TABLE lt_grpfld.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
