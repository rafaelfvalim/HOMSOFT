...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_grpfld_s.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_grpfld_s TYPE STANDARD TABLE OF zhms_tb_grpfld_s,
      ls_grpfld_s LIKE LINE OF lt_grpfld_s.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        MOVE waxls-value TO ls_grpfld_s-codgf.
      WHEN '2'.
        MOVE waxls-value TO ls_grpfld_s-seqnr.
      WHEN '3'.
        MOVE waxls-value TO ls_grpfld_s-tabss.
      WHEN '4'.
        MOVE waxls-value TO ls_grpfld_s-fldss.
      WHEN '5'.
        MOVE waxls-value TO ls_grpfld_s-typef.
        APPEND ls_grpfld_s TO lt_grpfld_s.
        CLEAR ls_grpfld_s.
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tb_grpfld_s FROM TABLE lt_grpfld_s.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
