...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_grpfld_tx.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_grpfld_tx TYPE STANDARD TABLE OF zhms_tx_grpfld,
      ls_grpfld_tx LIKE LINE OF lt_grpfld_tx.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        MOVE waxls-value TO ls_grpfld_tx-codgf.
      WHEN '2'.
        MOVE waxls-value TO ls_grpfld_tx-spras.
      WHEN '3'.
        MOVE waxls-value TO ls_grpfld_tx-denom.
        APPEND ls_grpfld_tx TO lt_grpfld_tx.
        CLEAR ls_grpfld_tx.
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tx_grpfld FROM TABLE lt_grpfld_tx.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
