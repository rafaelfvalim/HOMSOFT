...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_mapping.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_mapping TYPE STANDARD TABLE OF zhms_tb_mapping,
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        MOVE waxls-value TO ls_mapping-codmp.
      WHEN '2'.
        MOVE waxls-value TO ls_mapping-derot.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tb_mapping FROM TABLE lt_mapping.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
