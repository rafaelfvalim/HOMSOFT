...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_messagin_tx.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_messagin_tx TYPE STANDARD TABLE OF zhms_tx_messagin,
      ls_messagin_tx LIKE LINE OF lt_messagin_tx.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        MOVE waxls-value TO ls_messagin_tx-natdc.
      WHEN '2'.
        MOVE waxls-value TO ls_messagin_tx-typed.
      WHEN '3'.
        MOVE waxls-value TO ls_messagin_tx-loctp.
      WHEN '4'.
        MOVE waxls-value TO ls_messagin_tx-mensg.
      WHEN '5'.
        MOVE waxls-value TO ls_messagin_tx-spras.
      WHEN '6'.
        MOVE waxls-value TO ls_messagin_tx-denom.
        APPEND ls_messagin_tx TO lt_messagin_tx.
        CLEAR ls_messagin_tx.
    ENDCASE.

  ENDLOOP.

  IF sy-subrc IS INITIAL.
    MODIFY zhms_tx_messagin FROM TABLE lt_messagin_tx.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
