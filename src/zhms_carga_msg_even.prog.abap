...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_msg_even.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_msg_even TYPE STANDARD TABLE OF zhms_tb_msg_even,
      ls_msg_even LIKE LINE OF lt_msg_even.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        IF ls_msg_even IS NOT INITIAL.
          APPEND ls_msg_even TO lt_msg_even.
          CLEAR ls_msg_even.
        ENDIF.
        MOVE waxls-value TO ls_msg_even-natdc.
      WHEN '2'.
        MOVE waxls-value TO ls_msg_even-typed.
      WHEN '3'.
        MOVE waxls-value TO ls_msg_even-loctp.
      WHEN '4'.
        MOVE waxls-value TO ls_msg_even-mensg.
      WHEN '5'.
        MOVE waxls-value TO ls_msg_even-event.
      WHEN '6'.
        MOVE waxls-value TO ls_msg_even-exevt.
        APPEND ls_msg_even TO lt_msg_even.
        CLEAR ls_msg_even.
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tb_msg_even FROM TABLE lt_msg_even.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
