...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_msge_vrs.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_msge_vrs TYPE STANDARD TABLE OF zhms_tb_msge_vrs,
      ls_msge_vrs LIKE LINE OF lt_msge_vrs.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        IF ls_msge_vrs IS NOT INITIAL.
          APPEND ls_msge_vrs TO lt_msge_vrs.
          CLEAR ls_msge_vrs.
        ENDIF.
        MOVE waxls-value TO ls_msge_vrs-natdc.
      WHEN '2'.
        MOVE waxls-value TO ls_msge_vrs-typed.
      WHEN '3'.
        MOVE waxls-value TO ls_msge_vrs-loctp.
      WHEN '4'.
        MOVE waxls-value TO ls_msge_vrs-mensg.
      WHEN '5'.
        MOVE waxls-value TO ls_msge_vrs-event.
      WHEN '6'.
        MOVE waxls-value TO ls_msge_vrs-versn.
      WHEN '7'.
        MOVE waxls-value TO ls_msge_vrs-ativo.
        APPEND ls_msge_vrs TO lt_msge_vrs.
        CLEAR ls_msge_vrs.
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tb_msge_vrs FROM TABLE lt_msge_vrs.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
