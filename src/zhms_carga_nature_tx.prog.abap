...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_nature_tx.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_nature_tx TYPE STANDARD TABLE OF zhms_tx_nature,
      ls_nature_tx LIKE LINE OF lt_nature_tx.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        MOVE waxls-value TO ls_nature_tx-natdc.
      WHEN '2'.
        MOVE waxls-value TO ls_nature_tx-spras.
      WHEN '3'.
        MOVE waxls-value TO ls_nature_tx-denom.
        APPEND ls_nature_tx TO lt_nature_tx.
        CLEAR ls_nature_tx .
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tx_nature FROM TABLE lt_nature_tx.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
