...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_pkgvld.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_pkgvld TYPE STANDARD TABLE OF zhms_tb_pkgvld,
      ls_pkgvld LIKE LINE OF lt_pkgvld.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        IF ls_pkgvld IS NOT INITIAL.
          APPEND ls_pkgvld TO lt_pkgvld .
          CLEAR ls_pkgvld.
        ENDIF.
        MOVE waxls-value TO ls_pkgvld-vldcd.
      WHEN '2'.
        MOVE waxls-value TO ls_pkgvld-derot.
        APPEND ls_pkgvld TO lt_pkgvld.
        CLEAR ls_pkgvld.
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tb_pkgvld FROM TABLE lt_pkgvld .
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
