...
*&
*&---------------------------------------------------------------------*
REPORT  zhms_carga_tx_regvld.

DATA: itxls     TYPE STANDARD TABLE OF kcde_cells,
      waxls     TYPE kcde_cells.

DATA: lt_tx_regvld TYPE STANDARD TABLE OF zhms_tx_regvld,
      ls_tx_regvld LIKE LINE OF lt_tx_regvld.
...
FORM insert_table .
  ...
  LOOP AT itxls INTO waxls.
    ...
    CASE waxls-col.
      WHEN '1'.
        IF ls_tx_regvld IS NOT INITIAL.
          APPEND ls_tx_regvld TO lt_tx_regvld.
          CLEAR ls_tx_regvld.
        ENDIF.
        MOVE waxls-value TO ls_tx_regvld-vldcd.
      WHEN '2'.
        MOVE waxls-value TO ls_tx_regvld-regcd.
      WHEN '3'.
        MOVE waxls-value TO ls_tx_regvld-spras.
      WHEN '4'.
        MOVE waxls-value TO ls_tx_regvld-ltext.
        APPEND ls_tx_regvld TO lt_tx_regvld.
        CLEAR ls_tx_regvld.
    ENDCASE.
  ...
  IF sy-subrc IS INITIAL.
    MODIFY zhms_tx_regvld FROM TABLE lt_tx_regvld.
    IF sy-subrc IS INITIAL.
      COMMIT WORK.
