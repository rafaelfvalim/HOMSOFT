FUNCTION zhms_envia_email_xped.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
*"     REFERENCE(EMAIL) TYPE  CHAR255
*"     REFERENCE(NNF) TYPE  CHAR20
...
******* Assign your Text  below ****************  *********************
    CLEAR wa_it_message.
    CONCATENATE 'Foi recepcionado em nosso sistema a nota fiscal:' lv_nota ' - do Fornecedor:' c2 '-que não contém no XML (ou estão com valores errados) as TAGS XPED (Númerodo Pedido de compras) e NITEM (Item do pedido de compras).'INTO
  wa_it_message-line SEPARATED BY space.
    APPEND wa_it_message TO it_message.
*** insert Blank Line{} *********************************************
    CLEAR wa_it_message.
